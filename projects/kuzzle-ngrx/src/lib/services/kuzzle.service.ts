...
import { Subject, Observable, of, from, Subscription, Observer, merge } from 'rxjs';
import { map, finalize, share, switchMap, catchError } from 'rxjs/operators';
...
export const KUZZLE_PROTOCOL = new InjectionToken<KuzzleAbstractProtocol>('KUZZLE_PROTOCOL');
export const KUZZLE_CONFIG = new InjectionToken<KuzzleModuleConfig>('KUZZLE_CONFIG');
...
export class KuzzleService {
  ...
  constructor(public kuzzle: Kuzzle, @Inject(KUZZLE_CONFIG) private kuzzleConfig: KuzzleModuleConfig) {
    ...
    this.eventSubscription = this.kuzzleEvent().subscribe(e => console.log('events', e));
  }
  ...
  public kuzzleEvent(): Observable<KuzzleModuleEvent> {
    ...
    this.kuzzle.on('disconnected', () => sendResult.next({ event: 'disconnected' }));
  ...
  public realtime(entityName: string, filters: ObjectWithAnyKeys): Observable<WebsocketEventDocument> {
    const sendResult = new Subject<WebsocketEventDocument>();
    let roomId: string | void;
    return from(
      this.kuzzle.realtime
        .subscribe(this.kuzzleConfig.index, entityName.toLowerCase(), filters, (eventObject: WebsocketEventDocument) =>
          sendResult.next(eventObject)
        )
        .catch(error => {
  ...
  public createEntity<T>(entityCollection: string, entity: T, id: string): Observable<T> {
    return from(this.kuzzle.document.create(this.index, entityCollection.toLowerCase(), entity, id)).pipe(
      map(r => ({ id: (r as KuzzleDocument<T>)._id, ...(r as KuzzleDocument<T>)._source }))
    );
  }
  public deleteEntity(entityCollection: string, id: string): Observable<string> {
    return from(this.kuzzle.document.delete(this.index, entityCollection.toLowerCase(), id)).pipe(
      map(i => i as string)
    );
  }
  public getEntityById<T>(entityCollection: string, id: string): Observable<T> {
    return from(this.kuzzle.document.get<T>(this.index, entityCollection.toLowerCase(), id)).pipe(
      map(r => ({ id: (r as KuzzleDocument<T>)._id, ...(r as KuzzleDocument<T>)._source }))
    );
  }
  ...
  public upsertEntity<T>(entityCollection: string, id: string, entity: T): Observable<T> {
    return from(this.kuzzle.document.createOrReplace(this.index, entityCollection.toLowerCase(), id, entity)).pipe(
      map(r => r as KuzzleDocument<T>),
      map(r => ({ id: r._id, ...r._source }))
    );
  }
  public search<T>(entityCollection: string, scrollOptions: ScrollOptions): Observable<T[]> {
    return from(this.kuzzle.document.search<T>(this.index, entityCollection.toLowerCase(), {}, scrollOptions)).pipe(
      map(d => (d as KuzzleSearchResult<T>).hits.map<T>(h => ({ id: h._id, ...h._source })))
    );
  }
}
